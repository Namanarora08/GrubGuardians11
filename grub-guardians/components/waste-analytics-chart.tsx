  ...
  return (
    <Card className="w-full backdrop-blur-md bg-white/5 border border-white/10 shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Amount of food waste (in kg) over the last 7 days.</CardDescription>
      </CardHeader>
      <CardContent>
        <div
        ...
        >
          {data.map((day) => (
            <div key={day.date} className="group relative flex w-8 flex-col items-center justify-end">
              <div
                className="w-full rounded-t-sm bg-grubGreen-500 transition-all duration-300 hover:bg-grubGreen-600"
                style={{ height: `${(day.amountKg / maxAmount) * chartHeight * 0.8}px` }} // Scale to 80% of chart height
              />
        ...
        <div className="mt-4 text-center text-sm text-muted-foreground">Food Waste (kg)</div>
      </CardContent>
    </Card>
  )
}
